...
inline void DBG_assert(bool bCondition, CHAR *pszFile, INT32 iLine)
{
    if (!bCondition)
    {
...
void DBG_trace(UINT uClass, CHAR *pszFile, INT32 iLine)
{
//    if (uClass & DBG_MASK)
...
void SIOPrintString(char *pszString, ...){
...
void DBG_dump(UINT uClass, BYTE *pData, UINT uLen)
{
    if ((uClass & DBG_MASK) && (DBG_INFO >= DBG_LEVEL))
    {
        ...
        for(i = 0; i<uLen; ++i)
        {
//FIXME            SIOPrintString("%02X ", pData[i]);
        }
//FIXME        SIOPrintString("\r\n");
    }
}
